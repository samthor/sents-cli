...
export const build = (cands, root) => {
  ...
  const resolvedGlobs = cands.map((cand) => {
    ...
    // "**/*.js". Notably this only seems to work on the left.
  ...
  if (updatedRoot.length > root.length) {
  ...
  debug && console.warn('really watching for', extendedRelativeGlobs, 'in', updatedRoot);
